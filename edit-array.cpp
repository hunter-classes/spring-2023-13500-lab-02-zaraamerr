...
int main(){
    ...
    do {
        ...
        for (int i = 0; i < SIZE; i++){ //for every int in the array
            std::cout << myData[i] << " "; //print it out to the terminal
        }
        std::cout << std::endl;
        std::cout << "Input index: " << std::endl; //asks user to input an index
        std::cin >> index; //stores user input into index
        if (index < 0 || index >= SIZE){ //checks if the index the user inputted is not within the appropriate range of the array
            std::cout << "Index out of range. Exit program" << std::endl; //if true, prints out this message
            return 0; //ends
        }
        std::cout << "Input new value: " << std::endl; //if the index is all good, ask user to input a new value to be put into that index
        std::cin >> v; //store user input into v
